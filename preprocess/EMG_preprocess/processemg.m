function  output = processemg( EMGRAW, fs, LOWPASSRATE,NUMPASSES )
...
EMGDC = EMGRAW - mean(EMGRAW); %均值应该为0
subplot(5,1,2);
plot(t,EMGDC);
xlabel('Time (s)');
...
EMGFILT = filtfilt(b,a,EMGDC); %带通滤波（去相位差后）
subplot(5,1,3);
plot(t,EMGFILT);
xlabel('Time (s)');
title('EMG Signal with Noise Removed');
%全波整流 full-wave rectification
EMGFWR = abs(EMGFILT); %全波整流，把正弦波的负值部分翻上去。
subplot(5,1,4);
plot(t,EMGFWR);
xlabel('Time (s)');
